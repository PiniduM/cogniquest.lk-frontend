...
const CompetitionCard: React.FC<IProps> = ({
  competitionData,
  callToAction,
  onClick,
}) => {
  ...
  const navigateToCompetition = () => {
    router.push(
      `/dashboard/host/manage_competition?competition_id=${competition_id}`
    );
  };
  ...
  return (
    <div className="grid px-2 py-2 bg-[var(--extraLightBg)] rounded-md">
      <div className="flex justify-between w-full mb-2">
        <h3 className="font-semibold text-xl">{competition_title}</h3>{" "}
